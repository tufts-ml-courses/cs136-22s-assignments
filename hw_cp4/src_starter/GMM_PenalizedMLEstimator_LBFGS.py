...
class GMM_PenalizedMLEstimator_LBFGS():
    ...
    def fit(self, x_ND, x_valid_ND=None, verbose=True):
        ...
        def calc_loss(vec_M):
            ...
            return (
                calc_negative_log_likelihood(x_ND, log_pi_K, mu_KD, stddev_KD)
                + calc_penalty_stddev(stddev_KD, self.variance_penalty_mode, self.variance_penalty_spread)
                ) / (N * self.D)
        ...
        def callback_update_history(cur_param_vec):
            ...
            if x_valid_ND is None:
                ...
                denom = x_valid_ND.shape[0] * x_valid_ND.shape[1]
                valid_neg_log_lik_per_pixel = calc_negative_log_likelihood(
                    x_valid_ND, log_pi_K, mu_KD, stddev_KD) / denom
