    ...
    for K in K_list:
        ...
        for seed in seed_list:
            gmm_em = GMM_PenalizedMLEstimator_EM(
                K=K, D=D, seed=seed,
                max_iter=max_iter,
                variance_penalty_mode=0.1, variance_penalty_spread=25.0, do_double_check_correctness=True)
            ...
            cur_score = gmm_em.history['valid_neg_log_lik_per_pixel'][-1]
        ...
        plt.savefig("results_many_EM_runs/viz_K=%02d_seed=best.png" % (K),
            bbox_inches=0, pad_inches='tight')
        test_neg_log_lik_per_pixel = best_gmm.calc_negative_log_likelihood(x_test_ND) / (x_test_ND.shape[0] * x_test_ND.shape[1])
        best_gmm.history['test_neg_log_lik_per_pixel'] = np.zeros(len(best_gmm.history['train_loss']))
        best_gmm.history['test_neg_log_lik_per_pixel'][-1] = test_neg_log_lik_per_pixel
        best_gmm.write_history_to_csv("results_many_EM_runs/history_K=%02d_seed=best.csv" % (K))
